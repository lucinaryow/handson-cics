/****************************************************************************/
/* Program name: RRWRITE.C                                                  */
/*                                                                          */
/* Purpose: This program uses the CA-Realia callable file system            */
/*          to create a relative file with 1000 records. The file           */
/*          is written backwards randomly by relative record                */
/*          number.                                                         */
/*                                                                          */
...
void main(void)
{
   ...
   /*************************************************************************/
   printf("Writing 1000 records randomly, key = 1000 -> 1\n");
   for(recnum = 1; recnum <= 1000; recnum++)
   {
       sprintf(rec_ptr->recnum, "%.4d", recnum);
       strncpy(rec_ptr->trailer, " ", 1);
       rc = RealiaFSWriteRel(&handle, recnum, FILE_REC_LEN);
       if(rc)
       {
